...
<template>
  <div class="grid gap-4 md:grid-cols-3">
    ...
    <label class="grid gap-2">
      <span class="font-mono text-sm font-black tracking-widest text-sky uppercase">{{ t('image.pdfFitMode') }}</span>
      <select :value="options.fitMode" class="focus-ring w-full border border-line bg-grid px-3 py-2 font-mono text-base font-bold text-ink" @change="emit('updateFitMode', $event)">
