...
export async function fileToImageData(file: File, maxWidth: number, maxHeight: number, preserveDimensions: boolean, crop: ImageCropSelection | undefined, resizeMode: ImageResizeMode, resizePercent: number): Promise<ImageData> {
  const bitmap = await createImageBitmap(file)
  // 手動裁切優先；沒有 crop 時就保留完整來源圖，再依模式縮放。
  const source = crop ? getManualCropRect(bitmap.width, bitmap.height, crop) : getSourceRect(bitmap.width, bitmap.height)
...
function getSourceRect(imageWidth: number, imageHeight: number): SourceRect {
